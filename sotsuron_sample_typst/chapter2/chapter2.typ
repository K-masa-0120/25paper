...
本研究では周期的に変化する環境を扱うが、これは"各ノードが隣接ノードへ遅延情報を持つ"というQ-routingの環境から着想を得た。また、Q-routingは通信ネットワーク内の動的経路探索を扱うが、本研究では移動時間の周期的変動を考慮した強化学習手法を提案し、時間依存の最適経路選択を可能にする点で異なる。
...
本研究では、周期的環境への適応という観点を澁谷氏らの研究から参考にした。しかし、本研究の焦点は、報酬の周期的変動だけでなく、移動時間が周期的に変化する環境における強化学習手法の構築に当てられる。時間変化を入力とする手法と過去の手法を比較し、周期変化を考慮した深層強化学習の適用可能性を探る。
